    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            if (endpoint != null)
            {
                ...
                // 只有当endpoint需要权限检查时，才获取用户权限
                if (requiredPermission != null || requireAnyPermission != null || requireAllPermissions != null)
                {
                    // 获取用户ID
                    ...
                    {
                        if (!Shared.Helpers.PermissionHelper.HasPermission(permissionCodes, requiredPermission.Permission))
                        {
                    ...
                    if (requireAnyPermission != null)
                    {
                        var hasAnyPermission = requireAnyPermission.Permissions.Any(perm =>
                            Shared.Helpers.PermissionHelper.HasPermission(permissionCodes, perm));
